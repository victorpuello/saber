...
export default function ExamSession() {
  ...
  // Read metadata stashed by the catalog page
  ...
  // Navigate to results when session finishes
  useEffect(() => {
  ...
  // finished → useEffect navigates away; show spinner while redirecting
  if (phase === "finished") {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center gap-4 bg-surface">
        <span className="material-symbols-outlined animate-spin text-4xl text-primary">
        ...
        <p className="text-sm font-semibold text-on-surface-variant">Calculando resultados…</p>
      </div>
    );
  }
      ...
      <header className="z-50 flex h-20 shrink-0 items-center justify-between bg-surface/80 px-8 shadow-[0_12px_40px_rgba(25,28,30,0.06)] backdrop-blur-xl">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="max-w-[200px] truncate text-base font-bold tracking-tight text-primary">
                {examMeta.title}
              </h1>
              <div className="mt-0.5">
                <ExamTypeBadge type={examMeta.exam_type} />
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span
              className={`material-symbols-outlined text-[18px] ${timerUrgent ? "text-amber-600" : "text-secondary"}`}
            >
